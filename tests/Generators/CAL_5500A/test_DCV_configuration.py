...
#   FREQUENCY
    ...
    def Generate_Function(self):
        ...
        ActualFunction = self.FUNCTION[str(self.device.inst.query(
            "FUNC?").split(",")[0]).replace("\n", "")]
    ...
    def Amplitude_P2(self):
        actual_secondary_amplitude = self.device.inst.query("OUT?").split(",")[
            2]
    ...
    def Zcomp(self):
        actual_Zcomp = self.device.inst.query(
            "ZCOMP?").split(',')[-1].replace("\n", "")
    ...
    def RTD_Type(self):
        actual_RTD_Type = self.device.inst.query(
            "RTD_TYPE?").split(',')[-1].replace("\n", "")
        return actual_RTD_Type

    def TC_Type(self):
        actual_TC_Type = self.device.inst.query(
            "TC_TYPE?").split(',')[-1].replace("\n", "")
